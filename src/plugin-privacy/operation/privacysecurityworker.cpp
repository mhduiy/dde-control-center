...
void PrivacySecurityWorker::setPremissionEnabled(int appItemIndex, int premission, bool enabled)
{
    ...
    qWarning() << "现在设置的是: " << item->name() << premission << enabled;
...
ApplicationItem *PrivacySecurityWorker::addAppItem(int dataIndex)
{
    ...
    static const QStringList s_excludeApp = {
        "/uengsine.", "/dde-computer.desktop", "/dde-control-center.desktop", "/dde-file-manager.desktop", "/dde-trash.desktop", "/deepin-manual.desktop", "/deepin-terminal.desktop", "/uos-recovery-gui.desktop",
    };
    ...
    for (const auto &path : s_excludeApp) {
        if (desktopFile.contains(path)) {
            qCInfo(DCC_PRIVACY) << "Exclude app path: " << desktopFile << ", name: " << name;
            return nullptr;
        }
    }
...
void PrivacySecurityWorker::setAppPermissionEnableByCheck(bool ok)
{
    while (!m_cacheAppPermission.isEmpty()) {   // 提前设置好了要设置权限的列表
        ...
        if (ok) {
            ...
            qWarning() << "可执行文件路径" << file << premission;
            if (file.isEmpty()) {
...
void PrivacySecurityWorker::setPermissionEnableByCheck(bool ok)
{
    while (!m_cachePermission.isEmpty()) {
        ...
        case ApplicationItem::CameraPermission: {
            QString file = m_model->premissiontoPath(ApplicationItem::CameraPermission);
            // 设置camera Entity
            if (!m_entityMap.contains(file)) {
